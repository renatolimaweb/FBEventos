...
//webservice eventos por categoria

mysql_select_db($database_criativo, $conexao);
$query_busca_categoriaNegocio = "SELECT * FROM negocio WHERE (id_localidade = $localidade OR id_localidade = 0) AND id_categoria_negocio = $categoria ORDER BY id_negocio DESC";
$busca_categoriaNegocio = mysql_query($query_busca_categoriaNegocio, $conexao) or die(mysql_error());
$row_busca_categoriaNegocio = mysql_fetch_assoc($busca_categoriaNegocio);
$totalRows_busca_categoriaNegocio = mysql_num_rows($busca_categoriaNegocio);
...
<title><?=$row_busca_categoriaNegocio["titulo_categoria_negocio"];?> | Guia Comercial | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?></title>
<meta name="dc.title" content="<?=$row_busca_categoriaNegocio["titulo_categoria_negocio"];?> | Guia Comercial | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>"/>
...
<link rel="image_src" type="image/jpeg" title="<?=$row_busca_categoriaNegocio["titulo_categoria_negocio"];?> | Guia Comercial | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>" href="conteudo/img/<?=$row_busca_interfaceweb["open_graph"];?>"/>
...
<meta content="<?=$row_busca_categoriaNegocio["titulo_categoria_negocio"];?> | Guia Comercial | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>" property="twitter:title">
<meta property="og:title" content="<?=$row_busca_categoriaNegocio["titulo_categoria_negocio"];?> | Guia Comercial | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>"/>
...
<?php include_once("analyticstracking.php") ?>
    ...
    <div style="background-image:url(img/grid.png); background-repeat:repeat;">
		...
		<div class="container">
			...
			<div class="section">
                <h1 class="section-title title"><?=$xmlCategoriaNegocioIndividual->titulo_categoria_negocio;?></h1>
				<div class="row">
                    <?php include("includes/categoriaNegocio.php"); ?>
					<div class="col-sm-9">
                        	   ...
                        	   <?
								...
								<div class="post">
									<div class="entry-header">
                                        ...
                                        <a target="_blank" href="<?=$linha["url_negocio"];?>">
											<img class="img-responsive" src="conteudo/img/<?=$linha["imagem_negocio"];?>" />
										</a>
                                        </div>
									</div>
									<div class="post-content">								
										<div class="entry-meta">
											<ul class="list-inline">
												<li class="publish-date"><a href="#"><i class="fa fa-tag"></i> <?=$tituloCategoria;?> </a></li>
											</ul>
										</div>
										<h2 class="entry-title">
											<a target="_blank" href="<?=$linha["url_negocio"];?>"><?=$linha["titulo_negocio"];?></a>
										</h2>
										<div class="entry-content">
											<p><?=substr($linha["desc_negocio"],0,150); ?>...</p>
										</div>
									</div>
								</div><!--/post-->
                                <? } ?>
                        ...
                        <div class="pagination-wrapper">
					        ...
					        $pag =  $i +1;							
					        if ($pag <> $pagina) {
					 			 print "<li><a href=\"$PHP_SELF?pagina=$pag&categoria=$categoria&localidade=$localidade\">$pag</a></li>";
					             } else {
