...
<html class="fixed">
	<head>
		...
		<title>Localidades | FestasBrasil | Clouds | Sistema Gerenciador</title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
					<header class="page-header">
						<h2><i class="fa fa-map-marker"></i> Localidades</h2>
					</header>
                    ...
                    <section class="panel">
						<div class="panel-body" style="padding-bottom:0;">
							<div class="input-group mb-md">
                             <a href="ControleLocalidade.php?tp=<?=$tipo_conteudo;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT localidade.id_localidade, localidade.titulo_localidade, localidade.status_localidade, localidade.id_estado, estado.id_estado, estado.titulo_estado FROM localidade, estado WHERE (localidade.titulo_localidade like '%$descpesquisa%') AND localidade.id_estado = estado.id_estado ORDER BY titulo_localidade DESC";
				} else { 
				    
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM plano WHERE id_plano = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
						$sql = "SELECT localidade.id_localidade, localidade.titulo_localidade, localidade.status_localidade, localidade.id_estado, estado.id_estado, estado.titulo_estado FROM localidade, estado WHERE localidade.id_estado = estado.id_estado ORDER BY id_localidade DESC";
				}
				...
				$codigo 			= $linha["id_localidade"];
				$titulo	        	= $linha["titulo_localidade"];
				$estado	        	= $linha["titulo_estado"];
				$status             = $linha["status_localidade"];
				...
				?>
                             <section class="panel">
                                    ...
                                    <div class="col-md-12 pt-sm pb-sm">
                                        <p class="pull-right">
                                         <a href="ControleLocalidade.php?cod=<?=$codigo;?>" title="Editar"><span class="btn btn-warning"><i class="fa fa-pencil-square"></i></span></a>
                                         <a href="ConsultaLocalidade.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fa fa-trash-o"></i></span></a>
                                        </p>
                                        <p class="pt-sm"><i class="fa fa-database"></i> #<?=$codigo;?> | <i class="fa fa-location-arrow"></i> #<?=$estado;?></p>
