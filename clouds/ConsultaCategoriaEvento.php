			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="panel">
						<div class="panel-body" style="padding-bottom:0;">
							<div class="input-group mb-md">
                             <a href="ControleCategoriaEvento.php?tp=<?=$tipo_conteudo;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						$sql = "SELECT * FROM categoria_evento WHERE (titulo_categoria_evento like '%$descpesquisa%') ORDER BY id_categoria_evento DESC";
				} else { 
				    
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM categoria_evento WHERE id_categoria_evento = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
						$sql = "SELECT * FROM categoria_evento ORDER BY id_categoria_evento DESC";
				}
				...
				$codigo 			= $linha["id_categoria_evento"];
				$titulo	        	= $linha["titulo_categoria_evento"];
				$status             = $linha["status_categoria_evento"];
				...
				?>
                             <section class="panel">
                                    ...
                                    <div class="col-md-12 pt-sm pb-sm">
                                        <p class="pull-right">
                                         <a href="ControleCategoriaEvento.php?cod=<?=$codigo;?>" title="Editar"><span class="btn btn-warning"><i class="fa fa-pencil-square"></i></span></a>
                                         <a href="ConsultaCategoriaEvento.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fa fa-trash-o"></i></span></a>
                                        </p>
