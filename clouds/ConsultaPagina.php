...
<html class="fixed">
	<head>
		...
		<title>Páginas | FestasBrasil | Clouds | Sistema Gerenciador</title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="panel">
						<div class="panel-body" style="padding-bottom:0;">
							<div class="input-group mb-md">
                             <a href="ControlePagina.php?tp=<?=$tipo_conteudo;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						...
						$sql = "SELECT * FROM pagina, localidade, estado WHERE (titulo_pagina like '%$descpesquisa%') AND pagina.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado ORDER BY titulo_pagina DESC";
						}
						if ($categoria_usuario_autentica <> 1) {
						$sql = "SELECT * FROM pagina, localidade, estado WHERE (titulo_pagina like '%$descpesquisa%') AND pagina.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado AND pagina.id_localidade = $localidade_usuario_autentica ORDER BY titulo_pagina DESC";
						}
				} else { 
					...
					if ($_REQUEST['Del'] == "del") {
					    ...
					    if ($categoria_usuario_autentica == 1) {
						$sql = "SELECT * FROM pagina, localidade, estado WHERE pagina.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado ORDER BY id_pagina DESC";
						}
						if ($categoria_usuario_autentica <> 1) {
						$sql = "SELECT * FROM pagina, localidade, estado WHERE pagina.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado AND pagina.id_localidade = $localidade_usuario_autentica ORDER BY id_pagina DESC";
						}
				...
				?>
                             <section class="panel">
                                    ...
                                    <div class="col-md-12 pt-sm pb-sm">
                                        <p class="pull-right">
                                         <a href="ControlePagina.php?cod=<?=$codigo;?>" title="Editar"><span class="btn btn-warning"><i class="fa fa-pencil-square"></i></span></a>
                                         <a href="ConsultaPagina.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fa fa-trash-o"></i></span></a>
                                        </p>
