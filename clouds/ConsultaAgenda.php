...
<html class="fixed">
	<head>
		...
		<title>Agenda | FestasBrasil | Clouds | Sistema Gerenciador</title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="panel">
						<div class="panel-body" style="padding-bottom:0;">
							<div class="input-group mb-md">
                             <a href="ControleAgenda.php?tp=<?=$tipo_conteudo;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descpesquisa = $_REQUEST['pesquisa'];
						...
						$sql = "SELECT * FROM agenda, localidade, estado WHERE (titulo_agenda like '%$descpesquisa%') AND agenda.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado ORDER BY titulo_agenda DESC";
						}
						if ($categoria_usuario_autentica <> 1) {
						$sql = "SELECT * FROM agenda, localidade, estado WHERE (titulo_agenda like '%$descpesquisa%') AND agenda.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado AND agenda.id_localidade = $localidade_usuario_autentica ORDER BY titulo_agenda DESC";
						}
				} else { 
				    
					if ($_REQUEST['Del'] == "del") {
						$codigo = $_REQUEST['codigo'];
						$sql = "DELETE FROM agenda WHERE id_agenda = '$codigo'";
						$resultado = mysql_query($sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
					    if ($categoria_usuario_autentica == 1) {
						$sql = "SELECT * FROM agenda, localidade, estado WHERE agenda.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado ORDER BY data_agenda DESC, id_agenda DESC";
						}
						if ($categoria_usuario_autentica <> 1) {
						$sql = "SELECT * FROM agenda, localidade, estado WHERE agenda.id_localidade = localidade.id_localidade AND localidade.id_estado = estado.id_estado AND agenda.id_localidade = $localidade_usuario_autentica ORDER BY data_agenda DESC, id_agenda DESC";
						}
				}
				...
				$codigo 			= $linha["id_agenda"];
				$titulo	        	= $linha["titulo_agenda"];
				$desc	        	= $linha["desc_agenda"];
				$tags	        	= $linha["tags_agenda"];
				$imagem	        	= $linha["imagem_agenda"];
				$status             = $linha["status_agenda"];
				...
				?>
                             <section class="panel">
                                    ...
                                    <div class="col-md-9 pt-sm pb-sm">
                                        <p class="pull-right">
                                         <a href="ControleAgenda.php?cod=<?=$codigo;?>" title="Editar"><span class="btn btn-warning"><i class="fa fa-pencil-square"></i></span></a>
                                         <a href="ConsultaAgenda.php?Del=del&amp;codigo=<?=$codigo?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fa fa-trash-o"></i></span></a>
                                        </p>
