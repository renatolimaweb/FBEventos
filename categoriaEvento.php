...
$query_busca_categoriaEvento = "SELECT * FROM evento WHERE (id_localidade = $localidade OR id_localidade = 0) AND id_categoria_evento = $categoria ORDER BY id_evento DESC";
$busca_categoriaEvento = mysql_query($query_busca_categoriaEvento, $conexao) or die(mysql_error());
$row_busca_categoriaEvento = mysql_fetch_assoc($busca_categoriaEvento);
$totalRows_busca_categoriaEvento = mysql_num_rows($busca_categoriaEvento);
...
<title><?=$row_busca_categoriaEvento["titulo_categoria_evento"];?> | Coberturas | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?></title>
<meta name="dc.title" content="<?=$row_busca_categoriaEvento["titulo_categoria_evento"];?> | Coberturas | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>"/>
...
<link rel="image_src" type="image/jpeg" title="<?=$row_busca_categoriaEvento["titulo_categoria_evento"];?> | Coberturas | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>" href="conteudo/img/<?=$row_busca_interfaceweb["open_graph"];?>"/>
...
<meta content="<?=$row_busca_categoriaEvento["titulo_categoria_evento"];?> | Coberturas | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>" property="twitter:title">
<meta property="og:title" content="<?=$row_busca_categoriaEvento["titulo_categoria_evento"];?> | Coberturas | <?=$row_busca_config["titulo_config"];?> | <?=$row_busca_localidade["titulo_localidade"];?> - <?=$row_busca_localidade["titulo_estado"];?>"/>
...
<?php include_once("analyticstracking.php") ?>
    ...
    <div style="background-image:url(img/grid.png); background-repeat:repeat;">
		...
		<div class="container">
			...
			<div class="section">
                <h1 class="section-title title"><?=$row_busca_categoriaEvento["titulo_categoria_evento"];?></h1>
				<div class="row">
                    <?php include("includes/categoriaEvento.php"); ?>
					<div class="col-sm-9">
							...
							<div class="left-content">
                               ...
                               <div class="section listing-news">
                        		...
                        		<?
                                    ...
                                    $datatrans = explode ("-", $linha["data_evento"]); 
                                    $data = "$datatrans[2]/$datatrans[1]/$datatrans[0]";
                                ?>
								<div class="post">
									<div class="entry-header">
										<div class="entry-thumbnail">
                                        <a href="album.php?evento=<?=$linha["id_evento"];?>&localidade=<?=$localidade;?>">
											<img class="img-responsive" src="imagensGaleria/<?=$linha["imagem_evento"];?>" />
										</a>
                                        </div>
									</div>
									<div class="post-content">								
										...
										<h2 class="entry-title">
											<a href="album.php?evento=<?=$linha["id_evento"];?>&localidade=<?=$localidade;?>"><?=$linha["titulo_evento"];?></a>
										</h2>
										<div class="entry-content">
											<p><?=substr($linha["desc_evento"],0,140); ?>...</p>
										</div>
									</div>
								</div><!--/post-->
                                <? } ?>
