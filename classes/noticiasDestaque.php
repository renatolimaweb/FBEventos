...
$xml->openTag('lista');
//Controlador do Loop
$pag_views = 3; //TOTAL DE REGISTROS POR PÁGINA//
if (!$pagina) {
   $pagina = 1;
} else {
   $pagina = $pagina;
...
$inicio = $mat * $pag_views;
	//Consulta SQL
	$sql = "SELECT * FROM noticia, categoria_noticia WHERE (noticia.id_localidade = '$localidade' OR noticia.id_localidade = 0) AND noticia.id_categoria_noticia = categoria_noticia.id_categoria_noticia AND posicao_noticia = 1 AND status_noticia = 1 ORDER BY data_noticia DESC";
