...
$xml->openTag('lista');
//Controlador do Loop
$pag_views = 5; //TOTAL DE REGISTROS POR PÁGINA//
if (!$pagina) {
   $pagina = 1;
} else {
   $pagina = $pagina;
...
$inicio = $mat * $pag_views;
	//Consulta SQL
	$sql = "SELECT * FROM evento, categoria_evento WHERE (evento.id_localidade = '$localidade' OR evento.id_localidade = 0) AND evento.id_categoria_evento = categoria_evento.id_categoria_evento AND evento.posicao_evento = 4 AND evento.status_evento = 1 ORDER BY evento.data_evento DESC";
